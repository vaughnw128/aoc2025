...
fn main() {
    ...
    operators = lines.pop().unwrap().split_whitespace().rev().collect::<Vec<&str>>();

    // get max line length
    let max_line_len = lines.iter().map(|l| l.len()).max().unwrap();
    let nums = lines.iter()
        .map(|line|
            format!("{}{}", line, " ".repeat(max_line_len - line.len())).chars().collect::<Vec<char>>()
        ).collect::<Vec<Vec<char>>>();

    // pivot the matrix for easier parsing
    ...
    pivoted.insert(0, vec![' '; pivoted[0].len()]); // allmighty zero row
    ...
    for line in pivoted.iter().rev() {
        if line.iter().all(|n| *n == ' ') {
            ...
            if op == "+" {
                ...
                let mut product = 1;
                for n in register_nums.iter() { product *= *n; }
                sum += product;
                register_nums.clear();
                count += 1;
            }
        } else {
            register_nums.push(line.iter().collect::<String>().trim().parse::<u64>().unwrap());
        }
    }
